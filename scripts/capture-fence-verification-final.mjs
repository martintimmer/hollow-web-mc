...
try {
  ...
  fs.writeFileSync("snapshots/oak-fence-editor-verified.png", Buffer.from(editorSs.data, "base64"));
  console.log("Saved snapshots/oak-fence-editor-verified.png");

  // 2. In-game view with oak fence placed and held in hand + blocks catalog visible
  ...
  await Runtime.evaluate({
    expression: `(() => {
      ...
      // Search "fence" in catalog search
      const searchInput = document.querySelector('input[placeholder*="catalog"]');
      if (searchInput) {
        searchInput.value = "fence";
  ...
  fs.writeFileSync("snapshots/oak-fence-game-verified.png", Buffer.from(gameSs.data, "base64"));
  console.log("Saved snapshots/oak-fence-game-verified.png");
