...
try {
  ...
  // Click Log In & Play
  await evalExpr(`(() => {
    const b = Array.from(document.querySelectorAll("button")).find(b => b.innerText.toLowerCase().includes("log in") || b.innerText.toLowerCase().includes("quick play"));
    if (b) b.click();
  })()`);
  ...
  for (let i = 0; i < 25; i++) {
    ...
    const ready = await evalExpr(`(() => {
      ...
      const loading = overlay && (overlay.innerText.toLowerCase().includes("generating") || overlay.innerText.toLowerCase().includes("account sign in"));
      return !loading;
    })()`);
    if (ready) {
