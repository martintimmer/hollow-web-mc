import { spawn } from "node:child_process";
import CDP from "chrome-remote-interface";
import fs from "node:fs";
import path from "node:path";

const START = parseInt(process.argv[2] ?? "1", 10);
const COUNT = parseInt(process.argv[3] ?? "32", 10);
const TAG = process.argv[4] ?? "row";
const SHOT_COUNT = parseInt(process.argv[5] ?? "4", 10);

const PORT = 9301;
const chrome = spawn("/usr/bin/chromium", [
  "--headless=new", "--no-sandbox", "--disable-dev-shm-usage",
  "--enable-unsafe-swiftshader", `--remote-debugging-port=${PORT}`, "about:blank"
], { stdio: "ignore" });
const sleep = (ms) => new Promise((r) => setTimeout(r, ms));
await sleep(1500);

try {
  const client = await CDP({ port: PORT });
  const { Page, Runtime } = client;
  await Page.enable(); await Runtime.enable();

  await Page.navigate({ url: "http://127.0.0.1:5450/?sim=1" });
  await sleep(7000);

  const res = await Runtime.evaluate({
    expression: `(async () => {
      const api = window.__sim?.api;
      const s = window.__sim?.s;
      if (!api || !s) return { ok:false };
      try { api.undo(); api.undo(); } catch {}
      const r = api.stampRun("texrow", (w) => {
        for (let i = 0; i < ${COUNT}; i++) w(2 + i * 2, 65, 0, ${START} + i);
      });
      s.player.fly = true;
      s.uiPaused = false; s.active = true; s.time = 6000; s.timeFlow = false;
      return { ok:true, stamp: r };
    })()`, awaitPromise: true, returnByValue: true
  });
  console.log("stamp:", JSON.stringify(res.result?.value));
  await sleep(6000);

  const grab = async (name, x) => {
    await Runtime.evaluate({
      expression: `(() => { const s = window.__sim.s; s.player.x = ${x}; s.player.y = 67.6; s.player.z = ${Number(process.env.Z || 14)}; s.player.yaw = 0; s.player.pitch = Number(process.env.PITCH || -0.13); return true; })()`, returnByValue: true
    });
    await sleep(1800);
    const dat = await Runtime.evaluate({
      expression: `(() => {
        const s = window.__sim?.s; const cv = s?.renderer?.domElement;
        return cv ? cv.toDataURL("image/jpeg", 0.92) : null; })()`, returnByValue: true
    });
    if (dat.result?.value) {
      fs.writeFileSync("snapshots/" + name, Buffer.from(dat.result.value.split(",")[1], "base64"));
      console.log("saved " + name);
    } else console.log("no capture " + name);
  };

  const perShot = Math.ceil(COUNT / SHOT_COUNT);
  for (let k = 0; k < SHOT_COUNT; k++) {
    const midIdx = k * perShot + Math.floor(perShot / 2);
    const x = 2 + Math.min(COUNT - 1, midIdx) * 2;
    await grab(`tex-${TAG}-${String.fromCharCode(65 + k)}.jpg`, x);
  }
  await client.close();
} catch (e) { console.error("err:", e); } finally { chrome.kill(); }
