// Real prod: press W while frozen → check uiPaused/active cleared (the wake).
import CDP from "chrome-remote-interface";
import { spawn } from "node:child_process";
import fs from "node:fs";
const chrome = spawn("/usr/bin/chromium", ["--headless=new","--no-sandbox","--disable-dev-shm-usage","--enable-unsafe-swiftshader","--remote-debugging-port=9451","about:blank"], { stdio: "ignore" });
...
try {
  ...
  const state = async () => (await ev(`(()=>{const s=window.__worldDbg?.getS?.();return s?{uiPaused:s.uiPaused, active:s.active, steering:s.steering, pauseOpen:s.pauseOpen, ridden:!!s.riddenAnimal}:null;})()`)).result?.value;
  console.log("before:", JSON.stringify(await state()));
  await ev(`window.dispatchEvent(new KeyboardEvent("keydown",{code:"KeyW"})); "w"`);
  await sleep(600);
  console.log("after W:", JSON.stringify(await state()));
  fs.writeFileSync("snapshots/prod-wake.json", JSON.stringify(await state()));
  await c.close();
