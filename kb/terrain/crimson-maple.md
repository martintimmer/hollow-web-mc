---
id: terrain/crimson-maple
title: "Crimson Maple Forest (custom biome)"
kind: terrain
wiki: null
...
status: absent
tags: [biome, terrain, crimson-maple]
...
| registry id | `crimson_maple` |
| vanilla | custom |
| tree | crimson |
| density | 0.015 |
| map color | rgb(216, 40, 29) |
| notes | Custom autumn biome, no vanilla equivalent |
...
| Concern | Where |
|---|---|
| Registry | `catalog/biome-registry.json` id `crimson_maple` (implemented, `kb` was null) |
| Selection | `src/game/terrain/biomes.ts` |
| Surface/scatter | `src/game/terrain/terrainGenerator.ts` |
| Visualizer | `seed-page/main.ts` (`/seed.html` links `/kb/terrain/crimson-maple`) |
...
- Set the biome's `kb:` field to `terrain/crimson-maple.md` once this entry is real.
