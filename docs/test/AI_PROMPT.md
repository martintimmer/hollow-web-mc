# AI Master Voxel Generation Task: t
...
Your goal is to inspect the uploaded reference images in `docs/test/` (which may be **real-world photos, game screenshots, street scenes, vehicles, buildings, or props**) and convert them into:
...
2. **A Full Scene Assembly Layout (`scene_test.json`)** positioning all objects together in proper relative coordinates and orientations.

## 2. Reference Images Staged in Project
- `docs/test/`

## 3. Architectural Intent & User Notes
Analyze all objects in the photos and faithfully recreate their scale, shape, color, and materials in watertight 3D voxels.
