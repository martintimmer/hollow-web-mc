/**
 * @file src/game/vehicles/createMockupCar.ts
 * V0 mockup: an open cabriolet (no roof) built from three primitives,
 * with a visible interior (seats, dashboard, steering wheel) and the driver
 * sitting high. Front faces -Z (the physics forward direction).
 */
...
export function createMockupCar(): VehicleModel {
  ...
  add(new THREE.BoxGeometry(0.05, 0.2, 3.6), trimMat, -0.9, 0.4, 0);
  // headlights — flush rectangular lens units (not glowing orbs) + point lights
  for (const sx of [-0.55, 0.55]) {
    ...
    const hl = new THREE.PointLight(0xfff2b0, 1.6, 7);
    hl.castShadow = false;
  ...
  for (const sx of [-0.55, 0.55]) {
    ...
    const tl = new THREE.PointLight(0xff3020, 1.2, 5);
    tl.castShadow = false;
  ...
  // ── doors — small cabrio doors hinged at front edge (65 deg open) ──────────
  ...
  doorL.add(doorLMesh);
  // door window frame (glass when closed, visible)
  ...
  // trunk interior floor (visible when open)
  ...
  // ── interior (visible — open car) ─────────────────────────────────────────
  add(new THREE.BoxGeometry(1.4, 0.05, 2.2), interiorMat, 0, 0.62, -0.4); // floor
  // dashboard across the interior, front (-Z) side + slanted top + cluster
  ...
  // steering wheel — grouped so it rotates around the column axis (GTA-style)
  ...
  // ── wheels — classic chrome dish (Cabriolet's signature look) ────────────
  ...
  return { root: group, wheels, steeringWheel: steeringGroup, wipers: { left: wiperL, right: wiperR } as any, trunkHinge, trunkLid, doors: { left: doorL, right: doorR }, seatY: 1.52 };
}
